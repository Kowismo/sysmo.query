    ...
    public function __construct($ts, $clientInfo, $mongoDB, $cfg, $ezApp) {
        try {
            ...
            if($ezApp->inGroup($cfg['registerGroups'], $clientInfo['client_servergroups']) && !$ezApp->inGroup($cfg['ignoredGroups'], $clientInfo['client_servergroups'])) {
                ...
                $otherChannelSystem = $mongoDB->privateChannels->findOne(['clientUniqueIdentifier' => $clientInfo['client_unique_identifier']]);
                if($otherChannelSystem) {
                    $logMessage("User has a channel in the other system (privateChannels), channel ID: " . $otherChannelSystem['channelId']);
                    ...
                    $mongoDB->privateChannels->deleteOne(['channelId' => (int)$otherChannelSystem['channelId']]);
                ...
                $checkChannel = $mongoDB->privateChannels2->findOne(['clientUniqueIdentifier' => $clientInfo['client_unique_identifier']]);
                ...
                if($checkChannel == null) {
                    ...
                    $privateChannels = $mongoDB->privateChannels2->find()->toArray();
                    ...
                    if($channelInfo) {
                        ...
                        $insertResult = $mongoDB->privateChannels2->insertOne([
                            'clientUniqueIdentifier' => $clientInfo['client_unique_identifier'],
                        ...
                        $ezApp->createLog($mongoDB, __CLASS__, $clientInfo['client_unique_identifier'], $clientInfo['client_nickname'], 'stworzył kanał prywatny o numerze ' . $channelNumber);
                    } else {
                    ...
                    if($channelExists) {
                        ...
                        $mongoDB->privateChannels2->deleteOne(['channelId' => (int)$checkChannel['channelId']]);
                        // Rekursiver Aufruf, um einen neuen Kanal zu erstellen
                        $logMessage("Restarting channel creation process");
                        new privateChannels2($ts, $clientInfo, $mongoDB, $cfg, $ezApp);
                    }
                }
            } else {
